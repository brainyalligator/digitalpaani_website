...
class ScrollCarousel {
    ...
    changeSlide(newSlideIndex) {
        ...
        const previousVideo = this.images[this.currentSlide].querySelector('video');
        if (previousVideo) {
            previousVideo.pause();
        }
        ...
        // Play new video
        const newVideo = this.images[this.currentSlide].querySelector('video');
        if (newVideo) {
            newVideo.currentTime = 0; // Reset to start
            newVideo.play().catch(err => console.warn('Video play failed:', err));
        }
